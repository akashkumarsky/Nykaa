...
const CartSidebar = ({ isOpen, onClose, setPage }) => {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div
                className={`fixed top-0 right-0 h-full w-full max-w-sm bg-white shadow-xl transform transition-transform duration-300 z-50 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
                role="dialog"
                aria-modal="true"
                aria-hidden={!isOpen}
            >
                    ...
                    {!loading && (!cart || cart.cartItems.length === 0) ? (
                        ...
                        <div className="flex-grow overflow-y-auto p-4 space-y-4">
                            {cart?.cartItems?.map(item => (
                                <div key={`${item.id}-${item.product.id}`} className="flex items-center gap-4">
                                    <img
