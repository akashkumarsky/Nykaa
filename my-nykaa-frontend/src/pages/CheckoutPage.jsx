...
const CheckoutPage = ({ setPage }) => {
    ...
    // Saved addresses (normalized to a consistent shape)
    // shape: { id: string, fullName, street, city, zip, label: string, textOnly: boolean }
            ...
            try {
                ...
                const normalized = (Array.isArray(data) ? data : []).map((item, idx) => {
                    if (typeof item === 'string') {
                        ...
                        return {
                            id: `str-${idx}`, // synthetic id for string addresses
                            fullName,
                            street,
                            city,
                            zip,
                    ...
                    if (item && typeof item === 'object') {
                        ...
                        return {
                            ...
                            city,
                            zip,
                            label: `${fullName}, ${street}, ${city}, ${zip}`.replace(/\s+,/g, ',').trim(),
                            textOnly: false,
                        };
                    }
                    return null;
                ...
                setAddressStatus(normalized.length > 0 ? 'found' : 'none');
            } catch (err) {
    ...
    const handlePayment = async (e) => {
        ...
        const hasFormAddress = address.fullName && address.street && address.city && address.zip;
        ...
        try {
            ...
            const options = {
                ...
                handler: async function (response) {
                    ...
                    // Build payload: prefer addressId if we have a DB id (textOnly=false)
                    const useAddressId = selected && !selected.textOnly; // only when backend provided a real id
                    const payload = {
                        items: orderItems,
                        ...
                        shippingAddress: useAddressId
                            ? null
                            : (selected
                                ? selected.label // a string address from backend (no id)
                                : `${address.fullName}, ${address.street}, ${address.city}, ${address.zip}`),
                        saveAddress: !selected && !!saveAddress, // only save if it's a brand new form address
                ...
                prefill: {
                    name: (addresses.find(a => a.id === selectedAddressId)?.fullName) || address.fullName || '',
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-lg shadow">
                    ...
                    {/* Choose Address (always works after normalization) */}
                    {addressStatus === 'found' && addresses.length > 0 && (
                        <div className="mb-6">
                            ...
                            {/* Small preview of the selected address */}
                            {selectedAddressId && (
                    ...
                    <div className="flex items-center my-4">
                        ...
                        <span className="px-3 text-gray-500 text-sm">Or Fill New Address</span>
                        <div className="flex-grow border-t border-gray-300"></div>
                    </div>

                    {/* Fill Address Form (independent from dropdown) */}
